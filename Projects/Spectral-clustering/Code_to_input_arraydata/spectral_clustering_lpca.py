...
# Visualization
def plot_spectral_clustering(sample_data, r, eps, eta, d, K):
    # Plot the input data
    ...
    fig.savefig('figure_output.png')

    fig = plt.figure(figsize=(6, 4))  # Create an empty figure.
    for i in range(K):
        plt.plot(result_data[:, 0][result_data[:, 2] == i], result_data[:, 1][result_data[:, 2] == i], "o",
                  markersize=1, c=color_list[i])
    fig.savefig('figure_single_output.png')
